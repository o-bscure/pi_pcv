...
'''
middle_pixels_gray = tube_gray[:, len(tube_image[1])//2]

first_white = 0
last_white = -np.inf
for i in range(len(middle_pixels_gray)-1, -1, -1):
    if middle_pixels_gray[i] > 235:
        if first_white == 0:
            first_white = i
        last_white = i
    elif (last_white - i) > 20:
        break
print(first_white, last_white)
print((first_white-last_white)/700*5.0)
plt.figure()
plt.plot(middle_pixels_gray, 'ko')
plt.show()
'''
...
while (threshold <= 255) & (thresh_push < 2):
    ...
    for cnt in contours:
        ...
        low_y = y + h
        print("passes into if statement: ", bool(w>20 and w<60 and low_y>200))
